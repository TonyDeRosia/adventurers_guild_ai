...
REQUIRED_WORLD_DIRS = (
    ...
    "lore", "dialogue", "intelligence", "builder", "colors",
)
REQUIRED_BUILDER_DIRS = ("audit", "history", "snapshots", "exports", "imports", "templates")
...
class WorldRegistry:
    ...
    def validate_world(self, world_id: str) -> None:
        ...
        errors: list[str] = []
        for dirname in REQUIRED_WORLD_DIRS:
            if not (root / dirname).is_dir():
                errors.append(f"Missing required folder: {dirname}/")
        for dirname in REQUIRED_BUILDER_DIRS:
            if not (root / "builder" / dirname).is_dir():
                errors.append(f"Missing required builder folder: builder/{dirname}/")

...
*[cm(x,("rtarget","target") if x == "btarget" else (),"builder","implemented",f"{x} Builder Mode command.",phase="Builder Mode",builder=True, usage=x) for x in "builder build goto btarget redit rstat rcreate rset rdesc rname rexits rfeature rdelete exedit excreate exset exdelete fedit fcreate fset fdesc fdelete oedit ocreate oset odesc odelete ostat medit mcreate mset mdesc mdelete mstat spawnedit spawncreate spawnset spawndelete spawnstat zstat astat wstat rooms rlist rfind rsearch rwhere home areas alist acreate aedit astat aset adelete zones zlist zcreate zedit zstat zset zdelete dig link unlink del delete mlist olist map rmap".split()],
